...
static void list_add_between(list_t *__new, list_t *prev, list_t *next) {
	next->prev = __new;
	__new->next = next;
	__new->prev = prev;
	prev->next = __new;
...
static void list_add(list_t *__new, list_t *head) {
	list_add_between(__new, head, head->next);
}

static void list_add_tail(list_t *__new, list_t *head) {
	list_add_between(__new, head->prev, head);
}

static void *list_delete(list_t *item) {
...
static int list_isempty(list_t *head) {
	return head->next == head;
}

static int list_isfirst(list_t *head, list_t *entry) {
	return entry->prev == head;
}

static int list_islast(list_t *head, list_t *entry) {
	return entry->next == head;
}

static void list_connect(list_t *head, list_t *tail) {
...
static int list_islisted(list_t *item) {
